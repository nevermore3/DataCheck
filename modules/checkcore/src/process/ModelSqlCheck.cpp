...
namespace kd {
    namespace dc {
        ...
        bool ModelSqlCheck::execute(shared_ptr<CheckErrorOutput> errorOutput) {
            try {
                if (m_pdb != nullptr) {
                    string db_file = DataCheckConfig::getInstance().getProperty(DataCheckConfig::DB_INPUT_FILE);
                    m_pdb->open(db_file);
                } else {
                    return false;
                }
                        ...
                        for (auto it : cItem.second) {
                            ...
                            if (GetCheckResults(dcItem.ID, dcItem.Sql, mFieldNameIndex, mCheckResults, details)) {
                                for (const auto &detail : details) {
                                    shared_ptr<DCSqlCheckError> sql_check_error = make_shared<DCSqlCheckError>(
                                            dcItem.ID);
